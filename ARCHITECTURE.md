...
```mermaid
flowchart TD
    A[Schedulers: GitHub Actions / Cron / API Trigger] --> B[Source Connectors]
    B --> C[Raw Data Lake]
...
- Sentiment score
- Cross-market score
- Trump/admin policy impact context
- Trend impact summary
- Event radar context for SNDK-style acceleration/exhaustion detection
- Bullish/bearish final verdict context
- Risk penalty
...
- [ ] Manual trade journal is separate from automatic dummy paper trades.
- [x] Policy theme watchlists do not override strict watchlist-only prediction scope.
- [x] Default active watchlist stays focused instead of scanning all scripts.
- [x] Browser/manual and Windows Task Scheduler collection jobs use the same retryable job runner.
