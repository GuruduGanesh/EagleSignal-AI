...
- Manual Trade Journal supports full add / edit / delete with live P/L re-marking (REST: POST/PUT/DELETE `/manual-trades`), plus **Add trade** buttons from Overview, Options Edge, and Trade Strategy that capture the displayed/current entry price for a user-tracked long/short/options idea
...
- Options Edge tab for short-term **index options only**: SPX, XSP, NDX, XND, RUT, VIX, DJX, and OEX. It uses multi-source live chains (yfinance with a keyless CBOE delayed-quotes fallback), a default **5-DTE minimum** (`MIN_OPTION_DAYS_TO_EXPIRY=5`), a **50+ underlying-index-point expected-move gate** (`MIN_INDEX_OPTION_MOVE_POINTS=50`), and the **top 3 expirations by confidence** per index. Each row has a plain **BUY CALL (up ▲) / BUY PUT (down ▼) / NO TRADE (→)** call, green/orange/red colors, underlying current price, reference option contract/premium, bid/ask, bid/ask spread %, exact contract volume/OI, approximate delta/theta/vega, breakeven, premium % of spot, IV/realized-vol ratio, IV Rank/Percentile once enough snapshots exist, ATM IV skew, term-structure slope, chain-derived unusual-activity score, exact-contract OI-change, readiness, option-quality score, lot size, volume, OI, put/call, ATM strike, defined-risk spread strikes, IV, and an algorithmic confluence score. The **Options Risk Gate** deep-scores directional conviction, data quality, algo confluence, exact-contract liquidity, DTE, IV/realized vol, IV Rank, Greeks/theta, flow, premium cost, and spread, then caps or downgrades weak rows to **spread only**, **paper only**, or **no trade**. Add-option buttons are labeled by gate (tradeable, spread leg, paper/watch) and always add only a tracked journal row, never a broker order. Index rows expand/collapse the ranked expiries, and sorting keeps every index group together.
- Scheduled economic-event impact is first-class in every prediction: FOMC, rule-derived non-farm payrolls, weekly jobless claims, curated official CPI/PPI/PCE/GDP/retail/PMI dates when added to `config/event_calendar.yml`, and live per-ticker earnings are analyzed for event count, high-impact count, risk level, directional effect, option-action preference, and confidence policy. The same object is stored in `economic_event_impact`, `confidence_trace.economic_event_impact`, and `trend_impact.economic_event_impact`, then shown in Trends, Confidence Traces, Why Suggested, CSV, and Markdown reports.
- The stock-market prediction engine adds a shared broad-tape read to every signal. It combines market regime, VIX, WTI oil, dollar pressure, global correlations, government/policy/geopolitical events, scheduled economic/company events, and market-wide Hacker News + official Seeking Alpha `market-news.xml` + `feed.xml` headlines from the last 24 hours only.
- Trade Summary and Trade Strategy tabs convert the same signal/options data into an execution-style research plan. **Trade Summary** is compact and best-first: each ticker is the parent row, stock-only rows show current price, target price, target days, stop loss, and exit price, and qualifying option expiries expand underneath the ticker. Strategy-promoted options must have DTE at or above `MIN_OPTION_DAYS_TO_EXPIRY` with a hard floor of 5 days, a quoted premium at or below `$50`, and estimated option premium profit potential at or above `MIN_OPTION_PROFIT_PCT` (default `5%`); those expiry rows include contract, strike, option entry/stop/exit, confidence/readiness/risk/liquidity/IV/Greeks context, and **Add option**. **Trade Strategy** keeps the full detailed table and uses the best qualifying lower-premium option when available; below-5% option ideas are downgraded to watch/paper and do not get Add-option buttons. Live-price refresh recalculates current/target/stop/exit/profit-potential cells in-place.
- The dashboard is now reduced to the research-first tabs you asked for: `Index Options`, `Trends & Impact`, `News & Evidence`, `Why Suggested`, `Global Market`, `Jobs`, and `MD Validation`. The active tab persists across reloads and the sticky toolbar keeps manual `Reload`, `Live prices`, and `Re-scan`.
- Theme Watchlists tab now merges static baskets with live verdict/trend data for scored tickers
- Scheduled local collection is now intentionally simple: one automatic run daily at **9:00 AM America/Chicago** via `scripts/install_windows_tasks_split.ps1` or `scripts/install_windows_task.ps1`. After that, refreshes and re-scans are manual from the dashboard Jobs tab. Each manual run still fans grouped source refreshes out in parallel, then re-runs the focused prediction pipeline with parallel per-ticker workers and per-ticker retry for transient provider failures.
...
- Trend impact summary showing price move, news volume/providers, evidence polarity, policy links, social signal, forecast tilt, and scheduled economic-event risk
...
- Browser trigger: open `/dashboard`, choose **Jobs**, then click **Generate from newest data** to refresh every source group in parallel, re-run predictions, and write a fresh dashboard/report.
- Browser refresh is now manual-first: use **Generate from newest data**, **Refresh ALL + analyze**, **Live prices**, or **Re-scan** when you want an update. There is no recurring browser-side auto-refresh loop anymore.
- CLI trigger: `python -m eaglesignal collect --strategy swing --horizon 5D`.
- Windows schedule: run `scripts/install_windows_tasks_split.ps1` once to register the single automatic **9:00 AM America/Chicago** scan (`EagleSignalAI-Daily9AM`). The older simple installer `scripts/install_windows_task.ps1` now does the same one-task schedule.
