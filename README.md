...
- Historical point-in-time snapshots are persisted after successful scans when `ENABLE_HISTORICAL_SNAPSHOTS=true`: compact prediction, evidence, selected options-chain/expiry, and IV JSONL logs plus a per-run JSON file under `data/historical_snapshots`. This starts the no-lookahead foundation for IV Rank, scorecards, feature stores, and future ML training. Status is visible at `/snapshots/status`; measured prediction outcomes are visible at `/reliability/scorecard` once forward bars exist.
